...
import { getStoredUser, resendVerificationEmail } from '../../services/userAuthService'
    ...
    <AccountLayout>
      <section className="grid gap-6 xl:grid-cols-2">
        <article className="rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-6 sm:p-8">
          ...
          <h1 className="mt-3 font-serif text-3xl">{user?.isEmailVerified ? 'Email Verified' : 'Verification Required'}</h1>
          <p className="mt-4 break-words leading-7 text-[var(--color-muted)]">{user?.isEmailVerified ? 'Your account is ready for secure checkout.' : `Verify ${user?.email || 'your email'} before completing payment.`}</p>
